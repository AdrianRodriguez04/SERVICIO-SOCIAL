// Lectura de Sensor DHT22 via Web + Endpoint /metrics para Prometheus (Versión ESP32-C3)
#include <WiFi.h>
#include <DHT.h>

#define pinDatos 04       // GPIO 4 en el ESP32-C3 Super Mini
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n***** ESP32-C3: Lectura DHT22 con Prometheus *****");
...
void loop() {
  ...
  if (requestLine.indexOf("GET /metrics") >= 0) {
    ...
    body += "# HELP dht22_temperature_celsius Temperatura leida por el sensor DHT22 y el ESP32C3\n";
    body += "# TYPE dht22_temperature_celsius gauge\n";
    body += "dht11_temperature_celsius{sensor=\"dht22\",node=\"esp32c3_mini\"} ";
    body += String(temperatura, 2);
    body += "\n";

    body += "# HELP dht22_humidity_percent Humedad relativa leida por el sensor DHT22 y el ESP32C3\n";
    body += "# TYPE dht22_humidity_percent gauge\n";
    body += "dht11_humidity_percent{sensor=\"dht22\",node=\"esp32c3_mini\"} ";
  ...
  //  Ruta / -> Pagina HTML
    ...
    client.println("<title>ESP32-C3 Monitor</title></head>");
    client.println("<body style='font-family: Arial; text-align: center;'>");
    client.println("<h2>ESP32-C3: Monitoreo DHT11</h2>");
